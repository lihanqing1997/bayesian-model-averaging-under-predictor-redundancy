...
\begin{abstract}
\rev{Bayesian model averaging} can spread posterior mass over many nearly interchangeable model supports. \rev{This creates a reporting problem. A ranked list of exact supports can look unstable or uninformative even when the broader redundancy region is strongly supported.} We study density-ratio posterior compression for a fixed unrestricted support posterior. A support kernel \(w:\Gamma_p\to[0,1]\) is a weight function for a hard or soft support-space region. Normalizing the posterior after multiplying by \(w\) gives a restricted posterior component, and mixtures of these components have an exact density ratio \(h_q\) relative to the unrestricted posterior. \rev{This ratio gives total variation, forward and reverse Kullback--Leibler divergence, predictive contraction, and fallback diagnostics for many summaries of the same posterior.} The construction covers fixed hard regions, structured kernels, posterior-cluster kernels, pooled-pruned hybrids, and top-\(M\) atoms. \rev{We prove independent-sample and blocked Markov chain Monte Carlo validation bounds, candidate-pool coverage results, structural redundancy examples, and an atom-region separation showing when top-\(M\) atom storage is inefficient.} The experiments use \rev{Gaussian linear Bayesian model averaging} for exact and large-\(p\) benchmarks, report \(q_0\)-screened frontiers to separate region compression from fallback use, and include a small logistic support-posterior algebra check after exact support enumeration. \rev{Together, these components provide a formal reporting and validation layer for a fitted reference posterior rather than a replacement for reference-posterior computation.}
\end{abstract}
...
\rev{Bayesian model averaging (BMA)} averages over supports instead of conditioning on a single selected model \citep{hoeting1999,forte2018}. \rev{This protects against the underestimation of model uncertainty, but predictor redundancy creates a summarization problem that subset averaging alone does not solve.} The problem occurs both in classical regimes with \rev{moderate model size \(p\)} and in high-dimensional sparse regimes where \(p\) may be comparable to or \rev{larger than sample size \(n\)}. A posterior can concentrate on simple models while spreading its mass over many near-substitute supports. \rev{Here a support is the binary vector \(\gamma\in\Gamma_p=\{0,1\}^p\) indicating which predictors are active. The support posterior is discrete because \(\Gamma_p\) is a finite set, and one exact support with positive posterior mass is an atom of the posterior measure.} The scientifically stable object may then be a region of support space, rather than any particular atom.

This paper treats that situation as a posterior-compression problem for a fixed unrestricted support posterior. We seek hard or soft support-space regions with low expected descriptor cost that preserve enough of the unrestricted posterior to be reported with quantitative distortion diagnostics. The practical regimes are easiest to see as recurring redundancy patterns rather than as one application. Figure~\ref{fig:practicalregimes} sketches three common cases.
...
The paper proceeds as follows. Section~\ref{sec:background} defines the reference support posterior and the benchmark BMA instance. Section~\ref{sec:identity} gives the hard-region special case. Section~\ref{sec:mixture} gives the density-ratio identities, validation results, and region-versus-atom theory. Section~\ref{sec:mainalgorithms} describes how dictionaries are built and checked. Section~\ref{sec:leanmainexperiments} gives the empirical evidence. Section~\ref{sec:discussion} discusses scope and limitations, and \rev{the appendix contains the proofs and additional supporting theorems.}
...
The proof is elementary but important. Retained mass is not an auxiliary approximation, but the exact data term in the region posterior. \rev{The cancellation in \eqref{eq:fampostalpha} means that the factor \(p_0(\F)\) used to normalize the restricted support prior is exactly removed by the factor \(p_0(\F)^{-1}\) in the region-conditional marginal likelihood. What remains is the unrestricted posterior mass \(\alpha(\F)\).} Posterior comparison between regions separates into two readable pieces. The term \(\alpha(\F)\) rewards compatibility with the unrestricted analysis, while \(e^{-\lambda c(\F)}\) rewards compression.
...
\rev{This section serves four primary purposes. First, it establishes the density-ratio identity. Second, it applies this identity to TV, KL, and predictive checks. Third, it outlines how learned dictionaries are validated on held-out posterior draws. Fourth, it provides simple structural conditions under which regions outperform atom lists. Supporting details regarding concentration, MCMC, optimization, and covers are collected in Appendix~\ref{app:supportingtheory}.}
...
\rev{The following display is a definition of the storage-distortion curve for a chosen region list, not a theorem about the optimal list.}
...
\]
The empirical tables therefore report expected code together with active kernels, effective kernels, stored atoms, and candidate-list diagnostics when those quantities are available. Expected code measures a mixture-weighted reporting cost. It is not the same as the total cost of listing every active descriptor. We treat \(q_0\) as part of the storage-distortion reading. A low FKL value with large \(q_0\) is partial compression, because much of the approximation is still the unrestricted posterior. The conventions are fixed before fitting \(q\) and are used for all methods under the same comparison. \rev{Table~\ref{tab:storagecodes} records the reporting-code conventions used throughout the experiments.}

\begin{table}[!htbp]
\centering
\scriptsize
\input{sim/output/tables/table_storage_code_definitions.tex}
...
\rev{The validation results separate three checks. First, the reference posterior must be trustworthy. Second, the generated region list must be rich enough for the posterior geometry. Third, the optimizer must reach a small measured gap.} Table~\ref{tab:assumptiondiagnostics} spells out which assumptions are diagnostic gates and which are structural conditions that can be proved in redundancy regimes.
...
The cover condition is abstract, but it has concrete examples. \rev{The appendix gives matching statements for separated posterior Hamming balls, group-representative regions, and ordered intervals. These are the regimes behind the cluster-kernel, group-region, and spectroscopy examples.}
...
The remaining technical ingredients are standard enough to keep out of the main flow. \rev{Appendix~\ref{app:supportingtheory} collects the retained-mass concentration bound, the one-good-region implication, and the convex active-set optimization facts used by the algorithm.}
...
The experiments answer four questions that match the paper's main claims. First, can density-ratio summaries be checked exactly in redundant support spaces? Second, do region summaries improve on atom lists such as top-\(M\) truncation? Third, how do density-ratio dictionaries compare with nearby Bayesian baselines that change or summarize the support posterior? Fourth, do the conclusions survive a larger \(p=100\) run where the reference posterior is sampled and diagnosed?

The unrestricted BMA posterior is always the target being summarized. The tables therefore report TV, forward KL, reverse KL, fallback weight \(q_0\), expected descriptor cost, and active or effective list size when those quantities are defined. Smaller TV and FKL mean the summary is closer to unrestricted BMA on support space. A large \(q_0\) means that the summary still leans on the unrestricted fallback component. Lasso, elastic net, and group lasso are prediction baselines only, so their TV and FKL entries are not defined. Extra algebra checks, prediction-only rows, \(q_0\)-screened frontiers, saved-draw checks, and real-response reductions are collected in Appendix~\ref{app:empiricaldiagnostics}.

\subsection{Protocol and comparisons}
\label{subsec:benchmarktarget}

The benchmark separates dictionaries inside the density-ratio framework from methods that change the target posterior. Pooled-pruned and posterior-cluster kernels are learned support-region dictionaries. Top-\(M\) atoms, credible support sets, and fixed hard regions are limiting or response-independent support-kernel cases. Dilution-prior BMA and DPP-prior BMA are outside the framework because they change prior support weights before fitting BMA \citep{george2010,krishna2009}. Their TV and FKL columns are therefore target-shift diagnostics relative to the unrestricted benchmark, not evidence that those priors are poor for their own Bayesian targets.

Top-\(M\) truncation stores the largest posterior support atoms from ordinary BMA or stochastic model search \citep{hoeting1999,clyde2011}. An atom means one exact support point \(\gamma\). Posterior-cluster kernels cluster support samples under Hamming or group-Hamming distance and then use the density-ratio calculation to evaluate the resulting regions as posterior summaries. This connects to collinearity-aware and block-based posterior summaries \citep{ghosh2015collinearity,papaspiliopoulos2017}, but adds a fixed-target distortion check. Credible support sets store enough top supports to reach a posterior-mass target. Predictive sparse methods are reported only for RMSE and selected model size.

\subsection{Exact redundancy benchmark}
\label{subsec:currentexactcomparison}

The exact benchmark uses synthetic problems small enough to enumerate all \(2^p\) supports. It removes MCMC error from the comparison, so the reported TV, FKL, and RKL are exact posterior-compression errors. Table~\ref{tab:maincompetitorbenchmark} averages over two correlations and five replications in seven regimes, including one-representative redundancy, multi-representative redundancy, weak signals, noisy and misspecified grouping, ordered intervals, and graph communities. Each row selects the best FKL point in its tuning grid.

The result supports the density-ratio framework, but it does not say that one dictionary wins every metric. Pooled-pruned support kernels give consistently low FKL with \(q_0\) near the minimum fallback weight and moderate expected descriptor cost. Posterior-cluster kernels are strong raw-distortion summaries, while top-\(M\) atoms and credible support sets can be TV-competitive in small exact settings but store individual supports rather than regions. Dilution-prior BMA is competitive in some regimes, especially ordered intervals and weak signals, but it changes the posterior target.

\begin{table}[!tbp]
\centering
\scriptsize
\input{sim/output/tables/table_support_kernel_competitor_best_fkl.tex}
\caption{Exact dictionary benchmark. Parentheses give standard errors. Dilution and DPP rows are target-shift diagnostics because they change the prior target.}
...
\caption{Exact storage-distortion frontier for the fixed unrestricted posterior.}
\label{fig:maincompetitorfrontier}
\end{figure}

\subsection{Large support-space benchmark}
\label{subsec:largeendtoend}

The large benchmark moves to non-enumerable \(p=100\) support spaces. Each cell generates a synthetic redundant regression problem, runs six-chain MC3 to obtain an empirical unrestricted BMA reference posterior, checks the reference sample, and then compresses held-out posterior draws. The full rerun contains \(12\) scenario-correlation cells with three replications each.

Table~\ref{tab:largeendtoenddiag} appears before the compression table because the reference posterior is the object being compressed. The final rerun uses \(18{,}000\) retained cold-chain draws per cell. All \(36\) reference fits pass the core diagnostic gate, and one cell has a low-acceptance warning despite stable split-\(\widehat R\), ESS, and group-PIP diagnostics. The mean reference runtime was \(21.1\) seconds per cell, with range \(11.7\) to \(33.7\) seconds on the benchmark workstation. These timings quantify this benchmark rather than making a general speed claim.
...
Conditional on these reference samples, Table~\ref{tab:largeendtoendmethods} supports the intended rate-distortion claim. Pooled-pruned support kernels have lower FKL and lower expected descriptor cost than top-\(M\) atoms, dilution-prior BMA, DPP-prior BMA, credible support sets, and fixed hard regions. Cluster kernels give the lowest raw TV and FKL, but they leave larger fallback weight and require reporting learned medoids, bandwidths, and cluster counts. Thus the large benchmark supports the framework view rather than a one-size-fits-all default. Different dictionaries can be checked on the same density-ratio scale.

\begin{table}[!htbp]
\centering
\scriptsize
\input{sim/output/tables/table_large_end_to_end_askpc_family_summary.tex}
\caption{Large \(p=100\) benchmark. Storage is expected descriptor cost, and List is active kernels or stored atoms. \rev{Parentheses are standard errors when shown.} Dilution and DPP rows are target-shift diagnostics.}
\label{tab:largeendtoendmethods}
\end{table}

\subsection{Applied spectral check}
\label{subsec:appliedchecks}

\rev{The semi-synthetic spectroscopy experiment uses the real Tecator predictor matrix and a simulated response generated from a known banded signal. Thus the predictor geometry is real, while the active spectral region is known by construction.} This keeps the hard design feature, since adjacent wavelength channels are highly correlated, while giving a known active region for interpretability. Table~\ref{tab:mainsemisynth} and Figure~\ref{fig:mainsemisynth} show that structured pooled-pruned support kernels improve TV and forward KL over fixed hard-region lists and top-\(M\) atoms at low expected descriptor cost, with negligible predictive degradation. The learned high-weight kernels also overlap the simulated active band. The response is simulated, so the result is controlled practical validation rather than a full real-data claim.

\begin{table}[!htbp]
\centering
\input{sim/output/tables/table_adaptive_kernel_semisynthetic_realx.tex}
\caption{Semi-synthetic real-\(X\) Tecator compression with simulated response.}
...
\label{subsec:empiricaltakeaway}

The main experiments support three claims. The density-ratio calculation gives a common diagnostic scale for atom lists, hard regions, soft kernels, posterior clusters, and changed-prior comparisons. Region dictionaries are useful under predictor redundancy because they can summarize many nearly equivalent supports without listing each atom. The best dictionary depends on the reporting goal. Posterior-cluster kernels are strong when raw TV or FKL is the target, while pooled-pruned structured kernels give favorable expected-code support-region summaries when the fallback weight is small.

The companion GitHub repository contains the active R implementation, fixed seeds, tuning grids, preprocessing scripts, scripts for all main figures and tables, generated CSV and TeX output files, and documented commands for the exact benchmark, large \(p=100\) benchmark, logistic support-posterior check, \(q_0\)-screened frontier, reduced real-response benchmark, and PDF compilation.
...
\rev{This appendix collects supporting statements that are used by the main results but are not part of the main narrative. The preparatory identities and mass-estimation lemma appear first, followed by MCMC validation, finite-pool optimization, cover results, and atom-separation refinements.}
...
\rev{Condition on the construction split and on a finite candidate pool \(\mathcal W=\{w_0,\dots,w_J\}\) containing the fallback kernel \(w_0\equiv1\).} Let all retained masses be lower bounded by the floor \(a\), and let validation and alpha-estimation samples be independent of the construction split. Suppose the active-set solver returns \(\widehat q\in\Delta_{\epsilon_0}\) with empirical finite-pool optimization gap
\[
...
\label{app:empiricaldiagnostics}

This appendix keeps the empirical checks that are useful for auditability but not central enough for the main empirical argument. They verify the algebra, show atom-truncation scaling, record prediction-only baselines, inspect tuning behavior, and report secondary applied checks.

\subsection{Algebra and atom diagnostics}
\label{subsec:exactchecks}

Table~\ref{tab:mainsoftkernelidentity} checks the support-kernel identities in an enumerable posterior. It computes TV, forward KL, reverse KL, and \(E_{\pi_0}h_q\) both by direct finite sums and by the density-ratio formulas. The differences are numerical roundoff.

\begin{table}[!htbp]
\centering
\input{sim/output/tables/table_soft_kernel_identity.tex}
\caption{Exact support-kernel identity validation.}
\label{tab:mainsoftkernelidentity}
\end{table}

Theorem~\ref{thm:topmseparation} explains why keeping the largest support atoms can be inefficient in group-representative redundancy. Table~\ref{tab:maintopmsep} and Figure~\ref{fig:maintopmsep} show the scaling. Atom truncation must store a constant fraction of \(M^\star=m^s\) supports to achieve small TV, while one support region preserves the whole exchangeable class.

\begin{table}[!htbp]
\centering
\input{sim/output/tables/table_topm_atom_separation.tex}
\caption{Top-\(M\) atom separation under exchangeable group-representative redundancy.}
...
\caption{TV distortion of top-\(M\) atom truncation under exchangeable group-representative redundancy.}
\label{fig:maintopmsep}
\end{figure}

Table~\ref{tab:logisticcheck} repeats the compression calculation for a binary logistic support posterior. It is a logistic support-posterior check for the model-agnostic compression step, not a full GLM BMA benchmark.

\begin{table}[!htbp]
\centering
\small
\input{sim/output/tables/table_logistic_support_kernel_check.tex}
\caption{Small logistic support-posterior algebra check.}
\label{tab:logisticcheck}
\end{table}

\subsection{Prediction and fallback screens}

Tables~\ref{tab:exactpredictivebaselines} and~\ref{tab:largepredictivebaselines} report prediction-only sparse baselines. These rows help interpret predictive performance but do not define compressed support posteriors. \rev{The main conclusion is that predictive sparsity alone does not answer the posterior-compression question. Some sparse predictors have competitive RMSE, but they do not provide TV, FKL, RKL, retained mass, or fallback diagnostics for the support posterior.}

\begin{table}[!htbp]
\centering
\scriptsize
\input{sim/output/tables/table_support_kernel_predictive_baselines.tex}
\caption{Prediction-only baselines in the exact benchmark. These methods do not define support posteriors, so TV and FKL are not applicable. \rev{Parentheses are standard errors when shown.}}
...
\centering
\small
\input{sim/output/tables/table_large_end_to_end_predictive_baselines.tex}
\caption{Prediction-only baselines in the large \(p=100\) benchmark. These methods do not define support posteriors, so TV and FKL are not applicable. \rev{Parentheses are standard errors when shown.}}
\label{tab:largepredictivebaselines}
\end{table}

Table~\ref{tab:q0screenedfrontier} screens the large tuning grid by \(q_0\). It checks whether a dictionary family still works when large fallback solutions are excluded.

\begin{table}[!htbp]
\centering
\small
\input{sim/output/tables/table_q0_screened_frontier.tex}
\caption{\(q_0\)-screened large \(p=100\) frontier. This is a screen over the fitted tuning grid, not a separate constrained optimization run.}
...
The candidate-source ablation is a medium-mode diagnostic rather than main evidence. It asks which candidate sources help the validation objective, and it checks that the pooled diagnostic is not worse than its embedded pruned subset under the same objective.

\begin{table}[!htbp]
\centering
\scriptsize
\resizebox{\textwidth}{!}{\input{sim/output/tables/table_adaptive_kernel_ablation.tex}}
\caption{Candidate-source ablation.}
\label{tab:adaptiveablationdetail}
\end{table}

Table~\ref{tab:mainadaptiveconv} reports one representative sequential trace. The penalized objective drops as columns are added, while raw distortion and expected code can move in different directions.

\begin{table}[!htbp]
\centering
\scriptsize
\input{sim/output/tables/table_adaptive_kernel_convergence.tex}
\caption{Sequential column-generation trace.}
\label{tab:mainadaptiveconv}
\end{table}

\subsection{Saved-draw and real-response checks}
\label{subsec:largersaveddraw}

Table~\ref{tab:mainadaptivep100} keeps the older saved-draw \(p=100\) check as a secondary diagnostic. It evaluates compression on stored unrestricted BMA posterior draws rather than rerunning the reference sampler and predictive splits.

\begin{table}[!htbp]
\centering
\scriptsize
\input{sim/output/tables/table_adaptive_kernel_full_draws.tex}
\caption{Saved-draw \(p=100\) compression check.}
\label{tab:mainadaptivep100}
\end{table}

\subsection{Reduced exact real-response spectroscopy}
\label{subsec:realresponsereduced}

The reduced real-response experiment uses Tecator meat spectroscopy with fat response and Gasoline near-infrared spectra with octane response \citep{thodberg1993,liland2026pls}. It averages contiguous spectral blocks using only \(X\), then enumerates the exact BMA posterior on the reduced ordered-region design. This is not full-resolution spectroscopy. It is a real-response check under an exact reference posterior. \rev{Table~\ref{tab:realresponseexact} reports whether the same density-ratio summaries remain useful when the response is observed rather than simulated.}

\begin{table}[!htbp]
\centering
\scriptsize
\input{sim/output/tables/table_real_response_reduced_best_fkl.tex}
\caption{Reduced exact real-response spectroscopy benchmark. \rev{The design is reduced before enumeration, so the table is an exact real-response check rather than a full-resolution spectroscopy analysis.}}
\label{tab:realresponseexact}
\end{table}

Table~\ref{tab:keypairedcomparisons} gives paired comparison checks. The mean gap is comparison minus pooled-pruned, so positive FKL means pooled-pruned has lower forward KL, while positive storage means pooled-pruned has lower expected descriptor cost. \rev{The table is meant to show the direction and uncertainty of the main pairwise claims rather than to introduce a new benchmark.}

\begin{table}[!htbp]
\centering
\footnotesize
\input{sim/output/tables/table_key_paired_comparisons.tex}
\caption{Paired comparison checks.}
\label{tab:keypairedcomparisons}
\end{table}

\section{Proofs}

\rev{The proofs are ordered to match the mathematical dependencies. We first prove the retained-mass theorem and the hard-region bridge bounds. We then prove the density-ratio identities, the validation results, the pool-cover theorem, and the structural redundancy results. When a later theorem uses a preparatory lemma or proposition, the proof names that result explicitly.}
