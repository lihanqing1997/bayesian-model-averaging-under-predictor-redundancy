...
\caption{Large \(p=100\) benchmark. Storage is expected descriptor cost, and List is active kernels or stored atoms. Dilution and DPP rows are target-shift diagnostics.}
...
Tables~\ref{tab:exactpredictivebaselines} and~\ref{tab:largepredictivebaselines} report prediction-only sparse baselines. These rows help interpret predictive performance but do not define compressed support posteriors.

\begin{table}[!htbp]
\centering
\scriptsize
\input{sim/output/tables/table_support_kernel_predictive_baselines.tex}
\caption{Prediction-only baselines in the exact benchmark. These methods do not define support posteriors, so TV and FKL are not applicable.}
...
\caption{Prediction-only baselines in the large \(p=100\) benchmark. These methods do not define support posteriors, so TV and FKL are not applicable.}
...
The reduced real-response experiment uses Tecator meat spectroscopy with fat response and Gasoline near-infrared spectra with octane response \citep{thodberg1993,liland2026pls}. It averages contiguous spectral blocks using only \(X\), then enumerates the exact BMA posterior on the reduced ordered-region design. This is not full-resolution spectroscopy. It is a real-response check under an exact reference posterior.

\begin{table}[!htbp]
\centering
\scriptsize
\input{sim/output/tables/table_real_response_reduced_best_fkl.tex}
\caption{Reduced exact real-response spectroscopy benchmark.}
\label{tab:realresponseexact}
\end{table}

Table~\ref{tab:keypairedcomparisons} gives paired comparison checks. The mean gap is comparison minus pooled-pruned, so positive FKL means pooled-pruned has lower forward KL, while positive storage means pooled-pruned has lower expected descriptor cost.
